...
| # | Finding | Resolution |
|---|---|---|
| UI-4 | Analytics page lacks colors; needs circles and bars | New `components/reports/CompletionDonut.tsx` (SVG progress ring with center % and `role="img"` label, zero-total safe) and `components/reports/StatusBars.tsx` (per-status horizontal bars with counts + %, `To do` yellow / `In progress` teal / `Done` green) wired into a new "Completion overview" section on the Reports page via a responsive `.analytics-grid` (side-by-side on desktop, stacked under 640px). |
| UI-5 | Status colors leak risk: global classes/tokens with no scoping | All new analytics styles are prefixed and scoped in `app/globals.css` (`.analytics-*`, `.donut-*`, `.bar-*`, `.stat-tone-*`) reusing the existing `--pending-*`/`--completed-*`/`--primary` tokens, so they only apply inside the analytics components and never bleed onto tasks/activity UI. `StatCard` gained an opt-in `tone` prop (`default`/`todo`/`done`/`info` → colored top accent border); Reports page applies `info` to Total, `todo`/`info`/`done` to the by-status cards. |

### Branch: fix/remove-next-api-proxies
